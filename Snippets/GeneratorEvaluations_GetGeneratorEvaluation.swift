...
// limitations under the License.
  ...
  func sample(
    ...
    let response = try await client.getGeneratorEvaluation(
      request: GetGeneratorEvaluationRequest()
        .with {
          $0.name =
            "projects/\(projectId)/locations/\(locationId)/generators/\(generatorId)/evaluations/\(evaluationId)"
        }
    )
    ...
    static func main() async throws {
      do {
        ...
        try await sample(
          ...
          generatorId: "[placeholder]", evaluationId: "[placeholder]")
      } catch {
        print("Error: \(error)")
      }
    }
